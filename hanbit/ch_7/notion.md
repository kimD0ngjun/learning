...
- 즉, **모든 가중치가 패널티를 받음 → 너무 큰 계수를 부드럽게 줄임**
- 모든 특성이 조금씩 모델에 남아 있으며, **모델을 최대한 일반화**시키는 목적

**라쏘(Lasso)**
- **일부 특성의 가중치(기울기)를 0으로 만들 수 있는** 선형 회귀 모델
- 손실 함수에 가중치 절댓값(L1 규제) 항을 추가해 일부 계수를 0으로 수렴시키는 게 포인트
- 즉, **불필요한 특성이 모델에서 제거되어 패널티를 받음**
- **모델을 단순화**하고 **자동으로 변수 선택**을 하는 목적
### Tip) 과소적합과 과대적합
**과대적합**
- 모델이 너무 훈련 데이터에 맞춰져서 잡음까지 학습
- `train_score >> test_score`
- 새로운 데이터 일반화 성능 낮음

**과소적합**
- 모델이 너무 단순 → 훈련 데이터도 제대로 학습 못함
- `train_score ≈ test_score` 둘 다 낮음
- 데이터 구조를 제대로 잡지 못함

**적정 학습**
- 훈련과 테스트 모두 잘 맞음
- `train_score ≈ test_score` 둘 다 높음
- 일반화 성능 최적(균형 잡힘)